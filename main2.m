    ...
    while ~isempty(open_list)
        ...
        for dx = -1:1
            for dy = -1:1
                ...
                f = g + h;
                % Ignore if a neighbor node is already in the open list with a lower f value
...
function dummy_positions = sendPathToCoppeliaSim(vrep, clientID, path)
    ...
    for i = 1:size(path, 1)
        pos = path(i, :);
        x = (pos(2) - 1) * 0.25; % Calculate x coordinate
        y = (pos(1) - 1) * 0.25; % Calculate y coordinate
        z = 0; % z coordinate (plane)
